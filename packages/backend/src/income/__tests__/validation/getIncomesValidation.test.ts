...
describe('validateGetIncomesInput', () => {
  const validInput = {
  ...
  describe('when the userId field is invalid', () => {
    it('should throw a ValidationError for missing userId', () => {
      ...
      expect(() => validateGetIncomesInput(input)).toThrow(new ValidationError(VALIDATION_ERROR_USERID_MISSING))
    })

    it('should throw a ValidationError for invalid userId', () => {
      const input = { ...validInput, userId: 123 }
      expect(() => validateGetIncomesInput(input)).toThrow(new ValidationError(VALIDATION_ERROR_USERID_TYPE))
    })
  })
  ...
  describe('when the accountId field is invalid', () => {
    it('should throw a ValidationError for missing accountId', () => {
      ...
      expect(() => validateGetIncomesInput(input)).toThrow(new ValidationError(VALIDATION_ERROR_ACCOUNTID_MISSING))
    })

    it('should throw a ValidationError for invalid accountId', () => {
      const input = { ...validInput, accountId: 123 }
      expect(() => validateGetIncomesInput(input)).toThrow(new ValidationError(VALIDATION_ERROR_ACCOUNTID_TYPE))
    })
  })

  describe('when input is valid', () => {
    it('should not throw an error', () => {
      expect(() => validateGetIncomesInput(validInput)).not.toThrow()
    })
  })
})